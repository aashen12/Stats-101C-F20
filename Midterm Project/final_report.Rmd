...
The predictors for our first candidate model were selected by fitting a logistic regression model with all predictors and examining a correlation heat map. Based on the results from the logistic regression model fit, we examined all of the significant predictors from this model in the correlation heat map and ended up extracting the top 29 predictors that were both significant and had low correlations with other predictors. For instance, many of the predictors beginning with `Broad_` were statistically significant, but were very closely correlated with other predictors of the same `Broad_` family. This led us to only select the most significant predictor from this family that had the least correlation with other predictors of the data set. There were many predictors in this data set that were of the same family and closely correlated as well, so we repeated this methodology for all of the significant predictors of the same family.

To select a threshold, we calculated the weighted categorization accuracy using 5-fold cross-validation across arbitrarily set seeds and selected the model with the WCA closest to 0.78. Our training data was divided using an 80%/20% training/test split. The threshold with the most consistent performance across all seeds was **ASK ETHAN**. 

Our candidate model's WCA for the training data was `round(0.78242421, 3)`, which resulted in a WCA of `r round(0.82298, 3)` on the public leader board. We selected a threshold of **ASK ETHAN**...
