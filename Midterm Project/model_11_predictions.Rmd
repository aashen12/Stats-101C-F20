...
vars <- vars[, -c(1, 4, 32, 13, 18, 21, 29, 14, 20, 15, 30,
                  15, 24, 31, 38, 34, 41, 49, 50, 69, 87, 67,
                  68, 66, 71, 72, 75, 77, 78, 81, 83, 84, 93, 95, 96)]
cor_mtx = round(cor(vars[, names(vars) != "class"]), 2)
library(reshape2)
#reshape it
melted_cor_mtx <- melt(cor_mtx)
...
# Eliminate all but one of a set of variables highly correlated with one another
vars <- vars[, -c(4, 5, 15, 17, 10, 11, 13, 30, 35, 39, 49,
                  53, 58, 50, 64, 56)]
cor_mtx = round(cor(vars[, names(vars) != "class"]), 2)
library(reshape2)
#reshape it
...
cor_heatmap
which(abs(cor_mtx) > 0.5, arr.ind = TRUE)[which(abs(cor_mtx) > 0.5, arr.ind = TRUE)[, 1] != which(abs(cor_mtx) > 0.5, arr.ind = TRUE)[, 2], ]
set.seed(12)
...
csv_file <- data.frame(id = tests$id,
                       class = preds)
write.csv(csv_file, "modelpredictions11.csv", row.names = FALSE)
model_10 <- read.csv("modelpredictions10.csv")
table("BEST MOD" = model_10$class, csv_file$class)
```

```{r}
numeric_testing <- tests[, -99]
n_zeroes_test <- rep(NA, nrow(numeric_testing))

for(i in seq_len(nrow(numeric_testing))){
  row_i_zeroes <- 0
  for(j in seq_len(ncol(numeric_testing))){
    if(round(numeric_training[i,j], digits = 5) == 0){
      row_i_zeroes <- row_i_zeroes + 1
    }
  }
  n_zeroes_test[i] <- row_i_zeroes
}
hist(n_zeroes)
hist(n_zeroes_test, add = TRUE)
max(n_zeroes)
max(n_zeroes_test)
```
