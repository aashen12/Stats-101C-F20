...
```{r}
vars <- training %>% select(all_of(ind_terms), Polyphen2, class) # these are just the predictors it liked
...
csv_file <- data.frame("id" = tests$id,
                       "class" = preds)
write.csv(csv_file, "modelpredictions_large_model_2.csv", row.names = FALSE)
