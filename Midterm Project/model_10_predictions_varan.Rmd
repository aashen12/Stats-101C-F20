...
vars <- vars[, -c(1, 4, 32, 13, 18, 21, 29, 14, 20, 15, 30, 15, 24, 31, 38, 34, 41, 48, 50, 68, 86, 88, 71, 72, 78, 74, 76, 80, 82, 83, 92, 94, 95)]
cor_mtx = round(cor(vars[, names(vars) != "class"]), 2)
library(reshape2)
#reshape it
melted_cor_mtx <- melt(cor_mtx)
...
vars <- vars[, -c(4, 5, 15, 17, 11, 13, 14, 29, 39, 47, 48, 51, 49,
                  54, 55, 52, 58, 63, 65)]
cor_mtx = round(cor(vars[, names(vars) != "class"]), 2)
library(reshape2)
#reshape it
...
for (rand in ceiling(runif(10, min = 1, max = 10000000))) {
  ...
  scores <- numeric(101)
  for (i in seq(from = 0, to = 0.1, by = 0.001)) {
    lda_mod <- table("pred"=apply(preds, 1, classify, k = i), "obs" = vars_test$class)
    lda_mod
    scores[i*1000] <- score(lda_mod)
  }
  names(scores) <- seq(from = 0, to = 0.1, by = 0.001)
  plot(scores, ylim = c(0.7, 0.9))
...
set.seed(42)
 ...
 lda_mod <- table("pred"=apply(preds, 1, classify, k = 0.01), "obs" = vars_test$class)
  ...
  score(lda_mod)
```
From the correlation heatmap of 32 variables, it seems like only a few of them have moderate-high positive/negative correlations. We can try fitting a multiple linear regression model on all the predictors and seeing which are significant. Correlation can be a big problem here, but I think we can still get  something out of it. 

```{r}
library(boot)

terms_glm <- glm(class ~ ., data = vars)
```
# Stuff to check
- matrix of correlation between all the 32 variables (can prob just use the cor() function)
- Brute force test models (set seeds for partitions, write a function to obtain score, store and find best model for each of 1 variable, 2 variables, 31 variables, 32 variables)
...
csv_file <- data.frame("id" = tests$id,
                       "class" = preds)
#write.csv(csv_file, "modelpredictions9.csv", row.names = FALSE)
