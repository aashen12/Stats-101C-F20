...
knn_ft <- train(class ~ ., data = vars_train, method = "knn", preProc = c("center", "scale"),
                trControl = train_cont, tuneGrid = expand.grid(k = 40))
for (k in 40) {
  print(k)
...
preds <- predict(knn_ft, newdata = tests, type = "prob")
preds <- apply(preds, 1, classify)
names(preds) <- tests$id
csv_file <- data.frame("id" = tests$id,
                       "class" = preds)
 write.csv(csv_file, "modelpredictions5_knn.csv", row.names = FALSE)
