---
title: "Model 12 Predictions REVISED"
...
# Revision: Removed more extraneous observations (switched threshold from needing 50 or fewer zeros to half of the total number of predictors)
...
tidymn <- tidymn %>% filter(p.value < 0.05 / ncol(tidymn))
...
## Thresholds vs. WCA to see best threshold
...
csv_file <- data.frame(id = tests$id,
                       class = preds)
#write.csv(csv_file, "modelpredictions12_updated.csv", row.names = FALSE)
best_mod <- read.csv("modelpredictions12_updated.csv")
